...
test.describe('Policy Feed', () => {
  ...
  test('empty states - no results message', async ({ page }) => {
    ...
    // Drag the impact slider handle to max (simulate user drag)
    const slider = page.locator('[data-testid="impact-min-slider"]');
    const box = await slider.boundingBox();
    if (box) {
      // Click near the far right to set a high threshold
